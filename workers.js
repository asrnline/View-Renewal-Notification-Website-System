...
<head>
  ...
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 448 512'><path fill='%236366f1' d='M152 24c0-13.3-10.7-24-24-24s-24 10.7-24 24V64H64C28.7 64 0 92.7 0 128v320c0 35.3 28.7 64 64 64H384c35.3 0 64-28.7 64-64V128c0-35.3-28.7-64-64-64H344V24c0-13.3-10.7-24-24-24s-24 10.7-24 24V64H152V24zM48 192H400V448c0 8.8-7.2 16-16 16H64c-8.8 0-16-7.2-16-16V192zm176 56c-13.3 0-24 10.7-24 24v80c0 13.3 10.7 24 24 24h80c13.3 0 24-10.7 24-24V272c0-13.3-10.7-24-24-24H224z'/></svg>">
  <style>
    .login-container {
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div id="loginBgPreview" class="mt-1 h-20 w-full rounded-md border border-gray-300 flex items-center justify-center text-white font-medium">
                登录页面背景预览
              </div>
            </div>
          </div>
        ...
        <div class="border-b border-gray-200 pb-6">
          ...
          <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label class="inline-flex items-center">
                <input type="checkbox" name="enabledNotifiers" value="weixin" class="form-checkbox h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                <span class="ml-2 text-sm text-gray-700">企业微信</span>
              </label>
            </div>
          ...
          <div id="weixinConfig" class="config-section">
            <h4 class="text-md font-medium text-gray-900 mb-3">企业微信 配置</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label for="wecomCorpId" class="block text-sm font-medium text-gray-700">企业ID</label>
                <input type="text" id="wecomCorpId" placeholder="企业微信管理后台获取的企业ID" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              </div>
              <div>
                <label for="wecomAgentId" class="block text-sm font-medium text-gray-700">应用ID</label>
                <input type="text" id="wecomAgentId" placeholder="企业微信应用的AgentId" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              </div>
              <div>
                <label for="wecomAppSecret" class="block text-sm font-medium text-gray-700">应用Secret</label>
                <input type="text" id="wecomAppSecret" placeholder="企业微信应用的Secret" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              </div>
              <div>
                <label for="wecomToUser" class="block text-sm font-medium text-gray-700">接收人</label>
                <input type="text" id="wecomToUser" placeholder="@all 或 UserID1|UserID2" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <p class="mt-1 text-sm text-gray-500">默认为 @all 表示发送给所有人，多个UserID用 | 分隔</p>
              </div>
            </div>
            <div class="flex justify-end">
              <button type="button" id="testWeixinBtn" class="btn-secondary text-white px-4 py-2 rounded-md text-sm font-medium">
                <i class="fas fa-paper-plane mr-2"></i>测试 企业微信 通知
              </button>
            </div>
          </div>
        </div>
  ...
  <script>
    ...
    async function loadConfig() {
      try {
        ...
        document.getElementById('wecomCorpId').value = config.WECOM_CORP_ID || '';
        ...
        document.getElementById('loginBgColorText').value = loginBgColor;
        updateBgColorPreview(loginBgColor);
    ...
    // 更新背景颜色预览
    function updateBgColorPreview(colorValue) {
      const preview = document.getElementById('loginBgPreview');
      if (colorValue.includes(',')) {
        // 渐变色
        const colors = colorValue.split(',');
        preview.style.background = 'linear-gradient(135deg, ' + colors[0].trim() + ' 0%, ' + colors[1].trim() + ' 100%)';
      } else {
        // 单色
        preview.style.background = colorValue.trim();
      }
    }
    
    // 添加背景颜色相关事件监听
    document.getElementById('loginBgColor').addEventListener('input', function(e) {
      const colorText = document.getElementById('loginBgColorText');
      if (colorText.value.includes(',')) {
        // 如果是渐变色，替换第一个颜色
        const colors = colorText.value.split(',');
        colors[0] = e.target.value;
        colorText.value = colors.join(',');
      } else {
        // 如果是单色，直接替换
        colorText.value = e.target.value;
      }
      updateBgColorPreview(colorText.value);
    });
    
    document.getElementById('loginBgColorText').addEventListener('input', function(e) {
      updateBgColorPreview(e.target.value);
    });
    ...
    function toggleNotificationConfigs(enabledNotifiers) {
      ...
      const weixinConfig = document.getElementById('weixinConfig');

      // 重置所有配置区域
      [telegramConfig, notifyxConfig, webhookConfig, weixinConfig].forEach(config => {
      ...
      enabledNotifiers.forEach(type => {
        if (type === 'telegram') {
          ...
          weixinConfig.classList.remove('inactive');
          weixinConfig.classList.add('active');
        }
      });
    }
    ...
    async function testNotification(type) {
      ...
      if (type === 'telegram') {
        ...
        config.WECOM_CORP_ID = document.getElementById('wecomCorpId').value.trim();
        config.WECOM_APP_SECRET = document.getElementById('wecomAppSecret').value.trim();
        config.WECOM_AGENT_ID = document.getElementById('wecomAgentId').value.trim();
        config.WECOM_TO_USER = document.getElementById('wecomToUser').value.trim() || '@all';

        if (!config.WECOM_CORP_ID || !config.WECOM_APP_SECRET || !config.WECOM_AGENT_ID) {
          showToast('请先填写 企业微信 的企业ID、应用Secret和应用ID', 'warning');
          button.innerHTML = originalContent;
          button.disabled = false;
          return;
        }
    ...
    document.getElementById('testWeixinBtn').addEventListener('click', () => {
      testNotification('weixin');
    });
  ...
  async handleRequest(request, env, ctx) {
      ...
      if (method === 'POST') {
        try {
          ...
          const updatedConfig = {
            ...
            WECOM_CORP_ID: newConfig.WECOM_CORP_ID || '',
            WECOM_APP_SECRET: newConfig.WECOM_APP_SECRET || '',
    ...
    if (path === '/test-notification' && method === 'POST') {
      try {
        ...
        } else if (body.type === 'weixin') {
          const testConfig = {
            ...config,
            WECOM_CORP_ID: body.WECOM_CORP_ID,
            WECOM_APP_SECRET: body.WECOM_APP_SECRET,
            WECOM_AGENT_ID: body.WECOM_AGENT_ID,
            WECOM_TO_USER: body.WECOM_TO_USER
          };

          const message = '【测试通知】\n\n这是一条测试通知，用于验证企业微信通知功能是否正常工作。\n\n发送时间: ' + new Date().toLocaleString();
          const result = await sendWeComNotification(message, testConfig);
...
async function getConfig(env) {
  try {
    ...
    return {
      ...
      WECOM_APP_SECRET: config.WECOM_APP_SECRET || '',
      WECOM_AGENT_ID: config.WECOM_AGENT_ID || '',
      WECOM_TO_USER: config.WECOM_TO_USER || '@all',
      LOGIN_BG_COLOR: config.LOGIN_BG_COLOR || '#667eea,#764ba2',
...
async function sendWeComNotification(message, config) {
  try {
    if (!config.WECOM_CORP_ID || !config.WECOM_APP_SECRET || !config.WECOM_AGENT_ID || !config.WECOM_TO_USER) {
      console.error('[企业微信] 通知未配置，缺少必要参数');
      return { success: false, message: '企业微信通知未配置，缺少必要参数' };
    }
    
    console.log('[企业微信] 开始发送通知');
    
    // 第一步：获取访问令牌
    const tokenUrl = `https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=${config.WECOM_CORP_ID}&corpsecret=${config.WECOM_APP_SECRET}`;
    const tokenResponse = await fetch(tokenUrl);
    const tokenResult = await tokenResponse.json();
    
    if (!tokenResult.access_token) {
      console.error('[企业微信] 获取令牌失败:', tokenResult.errmsg);
      return { success: false, message: '获取令牌失败: ' + tokenResult.errmsg };
    }
    
    // 第二步：发送消息
    const sendUrl = `https://qyapi.weixin.qq.com/cgi-bin/message/send?access_token=${tokenResult.access_token}`;
    const messageData = {
    ...
    console.log('[企业微信] 发送结果:', sendResult);
    
    if (sendResult.errcode === 0) {
      return { success: true, message: '发送成功' };
    } else {
      return { success: false, message: '发送失败: ' + sendResult.errmsg };
    }
  } catch (error) {
    console.error('[企业微信] 发送通知失败:', error);
...
async function handleRequest(request, env, ctx) {
  ...
  if (loginBgColor.includes(',')) {
    // 渐变色
    const colors = loginBgColor.split(',');
         bgStyle = 'background: linear-gradient(135deg, ' + colors[0].trim() + ' 0%, ' + colors[1].trim() + ' 100%);';
  } else {
    // 单色
         bgStyle = 'background: ' + loginBgColor.trim() + ';';
  }
  
  // 替换登录页面模板中的背景样式
  const customLoginPage = loginPage.replace(
    '.login-container {\n      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);',
    `.login-container {\n      ${bgStyle}`
  );
  
  return new Response(customLoginPage, {
    headers: { 'Content-Type': 'text/html; charset=utf-8' }
  });
}
